...
def call_model(state: MessagesState, model):
    system_prompt = (
        "你是一个准确且有用的AI助手，要清晰区分用户和自己的身份，用一句话简洁地回答用户的问题。"
    )
...
def main():
    ...
    memory = ConversationBufferWindowMemory(k=5)  # k=5, 意味着能记住最后5轮对话内容
    RAG_TEMPLATE = """
    您是问答任务的助手，要准确使用以下检索到的上下文来回答问题。请清晰区分用户和自己的身份，如果你不知道答案，就说你不知道。最多使用三句话并保持答案简洁。
    <context>
    {context}
    </context>
    回答以下问题:
